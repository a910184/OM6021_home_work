...
volatile float y[100]={0};
volatile uint16_t datasin;
volatile uint16_t datacos;
ISR(TIMER0_COMP_vect){
    ...
    datacos=(uint16_t)2047*y[j]+2047;
    ASA_DAC00_put(ID_DAC, 0, 2, &datacos); //DAC卡輸出波型
    j++;
    if(j%99 == 0 ) {
        j = 0;
    }
...
int main() {
    ...
    for(int i =0; i<100; i++) {
        M128_HMI_get(d_bytes, &y[i]);  // send f32  array data
    }
    for(int i = 0; i<100; i++) {
        M128_HMI_put(d_bytes, HMI_TYPE_F32, &y[i]);    // send f32  array data
